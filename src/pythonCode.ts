/**
 * Generates the complete, standalone PySide6 Python script for Suno AI Library Backup
 */
...
export function generatePythonScript(options: {
  ...
  const genReport = pyBool(options.generateReport ?? true);
  const cookieVal = (options.cookieString || "").replace(/\\/g, "\\\\").replace(/"/g, '\\"');
...
class BackupWorker(QThread):
    ...
    def __init__(
        ...
        token: str,
        save_dir: Path,
        cookie_str: str = "${cookieVal}",
        download_audio: bool = ${dlAudio},
        preferred_format: str = "${preferredFormat}",
        prefer_wav: bool = ${prefWav},
        download_video: bool = ${dlVideo},
        download_image: bool = ${dlImg},
        download_metadata: bool = ${dlMeta},
        generate_report: bool = ${genReport},
        folder_grouping: str = "${grouping}",
...
class SunoBackupWindow(QMainWindow):
    ...
    def setup_dashboard_tab(self):
        ...
        self.cookie_input.setText("${cookieVal}")
        ...
        self.chk_meta.setChecked(${dlMeta})
        self.chk_report = QCheckBox("Offline Catalog (CSV/JSON)")
        self.chk_report.setChecked(${genReport})
